  ...
  return (
    <header className="fixed top-0 inset-x-0 z-50 isolate w-full max-w-full overflow-x-clip bg-background/95 backdrop-blur-lg border-b border-border/50 shadow-sm supports-[backdrop-filter]:bg-background/80">
      <div className="mx-auto flex h-16 w-full min-w-0 max-w-full items-center justify-between gap-2 px-3 sm:px-4">
        ...
        <Link
          to="/"
          className="flex min-w-0 flex-1 items-center gap-2 md:flex-none md:max-w-fit md:shrink-0 md:gap-3"
        >
          <img 
            src={logo} 
            alt="Intercambius" 
            className="h-8 w-8 shrink-0 md:h-10 md:w-10 rounded-full object-cover"
          />
          <span className="truncate text-base md:text-xl font-semibold gold-text">
            Intercambius
          </span>
        </Link>
          ...
          <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
            <SheetTrigger asChild>
              <Button
                variant="ghost"
                size="icon"
                className="md:hidden h-11 w-11 min-h-[44px] min-w-[44px] shrink-0"
